...
//uint8_t (*data_is_error_fun)(void); 数值错误判断函数 -> uint8_t  RC_Check_Data_IS_ERROR(void);
...
#define get_absolute(value) value >= 0 ? value : -value
...
int RC_DataProcess(volatile const uint8_t *pData, RC_ctrl_t *RC_CTRL) {
    ...
    RC_CTRL->rc.rocker_r_ = ((int16_t)pData[0] | ((int16_t)pData[1] << 8)) & 0x07FF;        //!< Channel 0
    RC_CTRL->rc.rocker_r1 = (((int16_t)pData[1] >> 3) | ((int16_t)pData[2] << 5)) & 0x07FF; //!< Channel 1
    RC_CTRL->rc.rocker_l_ = (((int16_t)pData[2] >> 6) | ((int16_t)pData[3] << 2) |          //!< Channel 2
                         ((int16_t)pData[4] << 10)) & 0x07FF;

    RC_CTRL->rc.rocker_l1 = (((int16_t)pData[4] >> 1) | ((int16_t)pData[5] << 7)) & 0x07FF; //!< Channel 3
    ...
    RC_CTRL->rc.dial = ((int16_t)pData[16]) | ((int16_t)pData[17] << 8); //左上角滑轮

    RC_CTRL->rc.rocker_r_ -= RC_CH_VALUE_OFFSET;
    RC_CTRL->rc.rocker_r1 -= RC_CH_VALUE_OFFSET;
    RC_CTRL->rc.rocker_l_ -= RC_CH_VALUE_OFFSET;
    RC_CTRL->rc.rocker_l1 -= RC_CH_VALUE_OFFSET;
    RC_CTRL->rc.dial -= RC_CH_VALUE_OFFSET;
		
	RC_CTRL->rc.rocker_r_ = rc_deadline_limit(RC_CTRL->rc.rocker_r_, 5); //死区限制
    RC_CTRL->rc.rocker_r1 = rc_deadline_limit(RC_CTRL->rc.rocker_r1, 5); //死区限制
    RC_CTRL->rc.rocker_l_ = rc_deadline_limit(RC_CTRL->rc.rocker_l_, 5); //死区限制
    RC_CTRL->rc.rocker_l1 = rc_deadline_limit(RC_CTRL->rc.rocker_l1, 5); //死区限制
    RC_CTRL->lost_time = 0;
    return 0;
}
...
void RC_DataReload(void) {
    ...
    rc_ctl.rc.rocker_r1 = 0;
    rc_ctl.rc.rocker_l_ = 0;
    rc_ctl.rc.rocker_l1 = 0;
    rc_ctl.rc.dial = 0;
    rc_ctl.mouse.x = 0;
    rc_ctl.mouse.y = 0;
    rc_ctl.mouse.z = 0;
...
uint8_t RC_Check_Data_IS_ERROR(void) {
    if (RC_abs(rc_ctl.rc.rocker_r_) > RC_CHANNAL_ERROR_VALUE) {
        goto error;
    }
    if (RC_abs(rc_ctl.rc.rocker_r1) > RC_CHANNAL_ERROR_VALUE) {
        goto error;
    }
    if (RC_abs(rc_ctl.rc.rocker_l_) > RC_CHANNAL_ERROR_VALUE) {
        goto error;
    }
    if (RC_abs(rc_ctl.rc.dial) > RC_CHANNAL_ERROR_VALUE) {
        goto error;
    }
    if (rc_ctl.rc.s1 == 0) {
        goto error;
...
/**
 * @brief 用于做键盘判断递增, 持续按下时速率逐步提高, 松开时逐渐降低, 但都不会超过最大最小的范围,  建议用于处理全局/静态变量
 * @param key_input 用于处理的按键
 * @param current_value 当前输入值
 * @param MAX_NUM 返回的最大值, 在 addordec 为 0 时松开按键, 往这个值递增
 * @param MIN_NUM 返回的最小值, 在 addordec 为 1 时松开按键, 往这个值递减
 * @param STATER_NUM 起步值, 在按下键值时不以 MIN_NUM(addordec == 1) 或者 MAX_NUM(addordec == 0) 启动
 * @param add_cycles 递增周期
 * @param dec_cycles 递减周期
 * @param addordec 1 按下增加, 0 为按下减少, 根据这个按键是想在按下时往正数增加还是负数增加来选择
 * @return 返回当前处理好的变量值
*/
// float key_board_up(uint8_t key_input, float *current_value, float MAX_NUM, float MIN_NUM, float STATER_NUM, float add_cycles, float dec_cycles, uint8_t addordec, uint32_t *lost_time)
float key_board_up(uint8_t key_input, float *current_value, float MAX_NUM, float MIN_NUM, float STATER_NUM, float add_cycles, float dec_cycles, uint8_t addordec)
{

    add_cycles = get_absolute(add_cycles);
    dec_cycles = get_absolute(dec_cycles);
    ...
    if(key_input)
    {
        if(addordec == 1)
        {
            ...
            {
                *current_value = MAX_NUM;
                return *current_value;
            }

            if( *current_value <  STATER_NUM )
            {
                *current_value = STATER_NUM;                
                return *current_value;
            }

            if(*current_value < MAX_NUM )
            {
                *current_value += ((MAX_NUM - MIN_NUM) / add_cycles);
                return *current_value ;
            }    
        }    
        else
        {
            ...
            if(*current_value > MIN_NUM)
            {
                *current_value += ((MIN_NUM - MAX_NUM) / dec_cycles);
                return *current_value ;
            }
        }
    }
    else
    {
        if(addordec == 1)
        {
            if(*current_value <= MIN_NUM)
            {
                ...
                *current_value -= (MAX_NUM - MIN_NUM) / dec_cycles;
            return *current_value;
        }
        else
        {
            ...
            *current_value -= (MIN_NUM - MAX_NUM) / add_cycles;
            return *current_value;
        }
    }
    return 0.0f;
